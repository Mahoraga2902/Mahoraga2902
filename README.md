# Hi, I'm Ian 👋
🔐 Cybersecurity Enthusiast | Computer Science Engineering Student

---

## 👨‍💻 About Me
- 🎓 CSE student with a strong interest in Cybersecurity
- 🔐 Focused on Web Security, Security Analytics & Threat Detection
- 🧠 Learning Python, Machine Learning & Network Security
- 🛠️ Hands-on experience with labs, CTF challenges, and mini projects
- 🎯 Aspiring Cybersecurity Analyst

---

## 🧰 Skills & Tools
![Skills](https://skillicons.dev/icons?i=python,linux,git,github,bash,vscode)

- Cybersecurity Tools: Burp Suite, Splunk, Wireshark
- Concepts: Web Application Security, Log Analysis, Fraud Detection
- OS: Linux (Mint, Ubuntu), Windows

---

## 🔐 Cybersecurity Focus Areas
- Web Application Security (SQL Injection, XSS, Authentication issues)
- Credit Card Fraud Detection using Machine Learning
- SIEM & Log Analysis with Splunk
- Basic Malware & Threat Analysis
- Capture The Flag (CTF) practice

---

## 📌 Featured Projects
- 🔍 **Credit Card Fraud Detection** – ML-based detection of suspicious card transactions  
- 🧠 **Deepfake Detection** – CNN-based image classification project  
- 📊 **Splunk SIEM Lab** – Log ingestion, dashboards, and analysis  
- 🧪 **Cybersecurity Learning Log** – Notes, labs, and hands-on practice  

---

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Mahoraga2902&show_icons=true)

---

## 📫 Connect with Me
- GitHub: https://github.com/Mahoraga2902
- LinkedIn: (to be added)

---

⭐ Always learning. Always improving.
